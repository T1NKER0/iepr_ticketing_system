...
const getUser = async (signInEmail, accessCredentials) => {
    try {
        ...
        const hashedInputPassword = crypto.createHash("sha256").update(accessCredentials).digest("hex");
...
// Function to get tickets for a specific user or all tickets for admin
const getTickets = (permission, uniqueId, callback) => {
    const query = permission === '1' ? process.env.SQL_GET_TICKETS_ADMIN : process.env.SQL_GET_TICKETS_USER;
    db.query(prepareQuery(query), [uniqueId], callback);
};
